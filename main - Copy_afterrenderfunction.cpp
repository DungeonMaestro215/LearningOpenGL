...
void renderScene(std::unordered_map<const char *, Shader> shaderMap, std::unordered_map<const char *, void *> settings, Texture diffuseMap, Texture specularMap, Model box, VBO instanceVBO, glm::mat4 *bigInstanceModels, glm::mat4 *instanceModels, Model backpack, VAO lightVAO, int numBoxes);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		renderScene(shaderMap, settings, diffuseMap, specularMap, box, instanceVBO, bigInstanceModels, instanceModels, backpack, lightVAO, numBoxes);
...
void renderScene(std::unordered_map<const char *, Shader> shaderMap, std::unordered_map<const char *, void *> settings, Texture diffuseMap, Texture specularMap, Model box, VBO instanceVBO, glm::mat4 *bigInstanceModels, glm::mat4 *instanceModels, Model backpack, VAO lightVAO, int numBoxes) {
		...
		shaderMap["lightShader"].SetVec3("lightColor", lightColor);
		...
		shaderMap["shaderProgram"].SetVec3("spotLight.position", camera.Position);
		shaderMap["shaderProgram"].SetVec3("spotLight.direction", camera.Orientation);
		...
		shaderMap["modelShader"].SetVec3("spotLight.position", camera.Position);
		shaderMap["modelShader"].SetVec3("spotLight.direction", camera.Orientation);

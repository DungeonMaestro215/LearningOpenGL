...
vec4 CalcPointLight(PointLight light, vec3 normal, vec3 fragPosition, vec3 viewDir);  
vec4 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPosition, vec3 viewDir);  
...
// Also, this should be adjusted so lighting does not affect the transparency lol
void main() {
...
vec4 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir) {
	...
	// Combine
	vec4 ambient = vec4(light.ambient, 1.0f) * texture(material.texture_diffuse0, texCoord);
	vec4 diffuse = (1.0f - shadow) * vec4(light.diffuse * diff, 1.0f) * texture(material.texture_diffuse0, texCoord);
	vec4 specular = (1.0f - shadow) * vec4(light.specular * spec, 1.0f) * texture(material.texture_specular0, texCoord);
...
vec4 CalcPointLight(PointLight light, vec3 normal, vec3 fragPosition, vec3 viewDir) {
	...
	// Combine
	vec4 ambient = vec4(light.ambient, 1.0f) * texture(material.texture_diffuse0, texCoord);
	vec4 diffuse = vec4(light.diffuse * diff, 1.0f) * texture(material.texture_diffuse0, texCoord);
	vec4 specular = vec4(light.specular * spec, 1.0f) * texture(material.texture_specular0, texCoord);
	ambient *= attenuation;		
	diffuse *= attenuation;
